# -*- coding: utf-8 -*-
"""
Created on Fri Apr 12 23:28:44 2019

@author: Usuario
"""

import gym, atari_py, random, cv2
import numpy as np
...
def make_env(env_id, env_conf):
    ...
    if "NoFrameskip" in env_id:
        ...
        env = MaxAndSkip(env, skip = env_conf["skip_rate"])
    ...
    if env_conf["normalized_observation"]:
...
def process_frame_84(frame, conf):
    ...
    frame = frame.mean(2)
    frame = frame.astype(np.float32)    # What? más ligero que un int???
    frame *= 1.0/255.0 
    frame = cv2.iesize(frame, (84, conf["dimension2"])) # Cambio de () a []
...
class NormalizedEnv(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        self.num_steps += 1
        self.mean = (self.mean * self.alpha) + obs.mean() * (1-self.alpha)
        self.std = (self.std * self.alpha) + obs.std() * (1 - self.std)
        
        unbiased_mean = self.mean/(1 - pow(self.alpha, self.num_steps))
        unbiasedS_std = self.std/(1 - pow(self.alpha, self.num_steps))
        
        return ((obs - unbiased_mean)/(unbiasedS_std + 1e-8))
...
class FireResetEnv(gym.Wrapper):
    def __init__(self, env):
        ...
        assert len(env.unwrapped.get_action_meaings()) >= 3
